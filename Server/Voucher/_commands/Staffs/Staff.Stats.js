...
module.exports = {
  ...
  onLoad: function (client) {
    ...
    client.ramalSaatYap = (date) => { return moment.duration(date).format('H'); };
  },
  ...
  onRequest: async function (client, message, args) {
    let barSystem = [
      ...
      {id: "diger", başlamaBar: "ramal_baslangicbar", doluBar: "ramal_dolubar", doluBitişBar: "ramal_dolubitisbar", boşBitişBar:"ramal_bosbitis", boşBar: "ramal_bosbar", başlangıçBar: "ramal_bosbaslama"},
    ]
    ...
    if(görevBul) {
      ...
      if(data.taskVoiceStats) {
        data.taskVoiceStats.forEach((value, key) => {
            genelses += value
            if(key == kanallar.publicKategorisi) public += Number(client.ramalSaatYap(value))
            if(key == kanallar.streamerKategorisi) public += Number(client.ramalSaatYap(value))
            if(key == kanallar.registerKategorisi) register += Number(client.ramalSaatYap(value))
        });
      };
        ...
        if(button.customId === "gorev_bilgisi") {
          ...
          if(görevBul.publicVoice >= 1) {
           ...
           if(görevBul.AllVoice >= 1) {
             embedcik.addFields({ name: `${görevBul.AllVoice} Saat Tüm Ses Kanallarında Takıl!`, value: `${Number(client.ramalSaatYap(genelses)) >= görevBul.AllVoice ? message.guild.emojiGöster(emojiler.Görev.OK) : message.guild.emojiGöster(emojiler.Görev.Mesaj) } ${yönetimBar(Number(client.ramalSaatYap(genelses)) ? Number(client.ramalSaatYap(genelses)) : 0, görevBul.AllVoice, 5, Number(client.ramalSaatYap(genelses)))} \`${Number(client.ramalSaatYap(genelses)) >= görevBul.AllVoice ? `Tamamlandı!`: `${Number(client.ramalSaatYap(genelses))} saat / ${görevBul.AllVoice} saat`}\``})
           }
