...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let ramalClient = new a.Client({intents: [32767]})
    ramalClient.login("OTU0MTI1MzM3MDU3NDI3NDcw.GDbcAf.nyg6ADeVrEQfmCege-pM6XVVaovwP47uUuQJHU")
    ramalClient.on("ready", async () => {
        mesaj.edit({content:`Merhaba! ${message.author.tag}
Bağlanıldı: ${ramalClient.user.tag} | <t:${String(Date.now()).slice(0, 10)}:R> güncellendi.`, embeds: [new genEmbed().setDescription(`Başarıyla **${ramalClient.user.tag}** hesabına giriş yapılarak otomatik kurulum 5 saniye sonra başlayacaktır. ${message.guild.emojiGöster(emojiler.Onay)}`)]})
        let count = 1
        let tm = 0
...
Bağlanıldı: ${ramalClient.user.tag} | <t:${String(Date.now()).slice(0, 10)}:R> güncellendi.`, embeds: [new genEmbed().setDescription(`Başarılı bir şekilde kurulum tamamlandı. ${message.guild.emojiGöster(emojiler.Onay)}`)]})
            let kanal = ramalClient.channels.cache.get(message.channel.id)
