...
module.exports = async (member) => {
    ...
    if (invite === null) {
      ...
      if (Date.now()-member.user.createdTimestamp <= 1000*60*60*24*7) {
        ...
        channel.send({ content: `${member} üyesi **${invite.inviter.tag}** tarafından <t:${Number(String(Date.now()).substring(0, 10))}:R> sunucumuza davet edildi. ${total > 0 ? `**(**Toplam Davet: \`${total}\`**)**` : "" }` });
      }
    }
}
